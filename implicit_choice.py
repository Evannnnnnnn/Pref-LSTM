...
for topic in topics:
    ...
    Constraints:
    - The agent must engage the user's by generating four sentences regarding {topic} for user to choose from regarding any topic.
    - The user must reply with a response that is one of the four choices.
    - Make each conversation very very vivid and different from typical examples. As much variation as possible.

    Example conversation:
    Agent:  Enroll in an in-person language course at a local community college or language school. Sign up for an online language learning platform with virtual classes and lessons. Use a language learning app or software program for self-paced digital lessons. Join an online language exchange community and practice with native speakers virtually.
    User: Enroll in an in-person language course at a local community college or language school.
    ...
    for i in range(100):
        ...
        try:
            ...
            if parsed:
                write_jsonl(parsed, "dataset/implicit_choice.jsonl")
